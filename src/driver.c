  ...
  for (d = alldevs; d; d = d->next, if_num++)
    for (a = d->addresses; a; a = a->next)
      if (a->addr && a->addr->sa_family == AF_INET) {
        match[if_num] = ip_prefix_match(
            ip, (uint8_t *)&((struct sockaddr_in *)a->addr)->sin_addr.s_addr);
        if (match[if_num] <
            ip_prefix_match((uint8_t *)&mask_all,
                            (uint8_t *)&((struct sockaddr_in *)(a->netmask))
                                ->sin_addr.s_addr))
          match[if_num] = 0;
      }
  if (if_num == 0) {
...
int driver_open() {
  ...
  // xn:
  // 混杂模式允许网卡接收通过网络传输的所有数据包，而不仅仅是它们的目的地址是网卡地址的数据包
  if ((pcap = pcap_open_live(if_name, 65536, 1, 10, pcap_errbuf)) ==
      NULL) //混杂模式打开网卡
  {
  ...
  // xn: 捕获目的 MAC 地址为 mac_addr 或者广播地址的数据包，并且排除源 MAC
  // 地址为 mac_addr 的数据包
  sprintf(filter_exp, //过滤数据包
          ...
          mac_addr[5], mac_addr[0], mac_addr[1], mac_addr[2], mac_addr[3],
 ...
 *
 */
void driver_close() { pcap_close(pcap); }
