...
int compare_do_pair(const void *a, const void *b) {
  ...
  if (pair_a->offset < pair_b->offset) return -1;
  if (pair_a->offset > pair_b->offset) return 1;
  return 0;
}
...
void ip_fragment_in(uint8_t* data, uint16_t len, uint16_t flags_fragment16, uint16_t id, uint16_t protocol, uint8_t* src_ip) {
  // 获取 mf 和 offset
  int mf = ((flags_fragment16 & IP_MORE_FRAGMENT) != 0);
  uint16_t offset = flags_fragment16;
  if (mf) offset -= IP_MORE_FRAGMENT;
  offset *= 8;
  ...
  if (!mf) {
    fv->is_over = true; // 表明已经收到了结束帧。注意此处不能直接整理然后 net_in ，因为可能结束帧乱序到达
  }
  ...
  if (fv->is_over) {
    ...
    // 排序中 offset 最大的帧的右边界与累积接收到数据大小相等，并且已经收到 mf 帧，说明接收完毕
    if (fv->bufs[fv->cnt - 1].offset + fv->bufs[fv->cnt - 1].len == fv->tot_size) {
      // 全部接收完毕，传递给上层 UDP
      buf_t new_buf;
...
void ip_in(buf_t *buf, uint8_t *src_mac) {
  ...
  ip_fragment_in(buf->data, buf->len, swap16(ip_hdr.flags_fragment16), swap16(ip_hdr.id16), protocol, ip_hdr.src_ip);
}
 ...
 */
void ip_init() { 
  map_init(&fragment_table, sizeof(uint16_t), sizeof(fragment_value), 0, IP_FRAGMENT_TIMEOUT_SEC, NULL);
